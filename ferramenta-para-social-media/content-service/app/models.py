...
conteudo_tecnicas = Table(
    "conteudo_tecnicas", Base.metadata,
    Column("conteudo_id", Uuid(as_uuid=True), ForeignKey("content.conteudos.id", ondelete="CASCADE"), primary_key=True),
    Column("tecnica_id", Uuid(as_uuid=True), ForeignKey("content.tecnicas.id", ondelete="CASCADE"), primary_key=True),
    schema="content",
)
...
class Conteudo(Base):
    ...
    tamanho_legenda: Mapped[str] = mapped_column(String(20), nullable=False, default="media")
    ...
    hashtags_padrao: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    usar_narrativa: Mapped[bool] = mapped_column(nullable=False, default=True)
    ...
    tecnicas_rel: Mapped[list[Tecnica]] = relationship(secondary=conteudo_tecnicas, lazy="selectin")
...
class GeracaoTexto(Base):
    ...
    __table_args__ = {"schema": "content"}
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    conteudo_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("content.conteudos.id", ondelete="CASCADE"), nullable=False, index=True)
...
class GeracaoImagemSalva(Base):
    ...
    geracao_texto_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("content.geracoes_texto.id", ondelete="SET NULL"), nullable=True, index=True)
...
class ImagemSalva(Base):
    ...
    __table_args__ = {"schema": "content"}
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    geracao_imagem_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("content.geracoes_imagem.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Marca(Base):
    ...
    __table_args__ = (UniqueConstraint("usuario_id", "cliente_id"), {"schema": "content"})
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    usuario_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
    cliente_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), nullable=True, index=True)
...
class NarrativaEstrategica(Base):
    __tablename__ = "narrativas_estrategicas"
    __table_args__ = (UniqueConstraint("usuario_id", "cliente_id"), {"schema": "content"})
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    usuario_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
    cliente_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
...
class LinhaEditorial(Base):
    __tablename__ = "linhas_editoriais"
    __table_args__ = (UniqueConstraint("usuario_id", "cliente_id"), {"schema": "content"})
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    usuario_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
    cliente_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
...
class LogoMarca(Base):
    ...
    __table_args__ = {"schema": "content"}
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    marca_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("content.marcas.id", ondelete="CASCADE"), nullable=False, index=True)
    arquivo: Mapped[bytes] = mapped_column(LargeBinary, nullable=False)
